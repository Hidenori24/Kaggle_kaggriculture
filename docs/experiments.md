...
## 2026-08-07: logistics-shadow-analyzer

- 現行リプレイを変更せず、各stepの観測値に対して`full_strategy`の提案だけを計算する読み取り専用解析を追加した。
- `scripts/analyze_logistics.py --episodes 3 --output <path>`で、資金、手数、区画、在庫、納屋在庫、作物・動物の未処理状態、実行行動とshadow提案の差分をJSONへ保存できる。
- 3シードでshadow計算は720/720 stepすべて例外なく完了した。現行リプレイの報酬は`197,329 / 187,784 / 192,641`（同一実行時）だった。
- 現行行動とshadow提案は全stepで異なり、shadowは初日から大量の作物・雇用・移動を提案する。一方、現行リプレイは日単位の雇用、土地、動物、物流をすでに統合している。このため、shadow案を部分的に上書きする方式は根拠がなく、直接採用しない。
- 次の改善は、shadowの提案をそのまま使うのではなく、現行リプレイの各日周期を壊さずに「未処理在庫が一定時間残る」「収穫可能なのに水・収穫が遅れる」など、観測上のボトルネックだけを抽出して小さく検証する。

## 2026-08-07: local-backlog-repair-rejected

- 解析で検出した未処理タイルに対し、既存行動が`PASS/移動`で、ユニットが対象タイル上にいる場合だけ、最大2ユニットの`HARVEST/WATER/FEED/CARE/COLLECT_FERTILIZER`へ置換する候補を検証した。
- 市場注文、雇用、移動先、運搬中在庫は変更していないが、5シードで基準`175,915 / 189,041 / 189,538 / 182,621 / 205,063`に対し候補`15,009 / 23,640 / 22,216 / 23,883 / 12,583`となった。
- 判断: 強者リプレイでは、同じタイル上の合法な局所処理でも、後続の搬送・収穫順序と結合している。局所repair方式は不採用・未提出。今後は候補行動の上書きではなく、日単位の完全な行動周期を複製できる場合だけ比較する。

## 2026-08-07: endgame-local-repair-rejected

- 終盤だけなら固定行動列への影響が小さい可能性を検証するため、最終`24 / 48 / 72 / 120`stepだけ局所repairを有効化した。
- 各条件を5シードで評価したが、基準版との差分平均はそれぞれ`-189,399.6 / -191,969.4 / -190,787.0 / -195,618.8`点で、全条件が悪化した。
- 終盤にも収穫・搬送・売却の連鎖が残っているため、終盤の1手置換も安全ではない。終盤判定を採用する場合は、検証済みの終盤行動列を丸ごと切り替える方式に限定する。

## 2026-08-07: evaluation-reproducibility

- `random`相手は`configuration.seed`だけでは相手の行動列が固定されず、同じseedでもbaselineの報酬が変動した（例: seed 0で`196,633`と`183,605`）。
- 固定PASS相手では、別プロセスで同じseedを2回実行しても`207,164 / 181,047 / 188,857`が完全一致した。安定版のagent・replay stateが原因で同一条件の結果が揺れているわけではない。
- `scripts/simulate.py`に`--opponent random|pass`を追加した。候補比較は`pass`で再現性を確認し、`random`は別途分布評価として扱う。

## 2026-08-07: stuck-override-rejected

- 数step同じ位置に留まったユニットだけ、別の合法な隣接移動へ切り替える非常用overrideを検証した。
- 5シードで基準`185,191 / 194,738 / 194,030 / 194,864 / 179,575`に対し候補`154 / 333 / 5,239 / 154 / 440`となった。
- `random`相手で全5シードが大幅悪化したため、overrideは不採用・未提出。1手の移動変更でもリプレイの後続周期を壊す可能性が高いと判断した。
