...
## 2026-08-12: revert-price-floor-restore-known-good

- 実測: 価格フロア導入後のKaggleスコアは`929.4`、shed圧力弁投入後も`約1,000`で停滞した。導入前は`1,900`（ピーク`2,500`）だったため、一連の価格フロア変更で約900点を失った。
- 検証手法の失敗（本件の本質）: `scripts/simulate.py`の相手は`random`で、ランダム相手は市場価格を暴落させない。そのため`_price_floor_fraction`はローカルではほぼ常に`1.0`を返し、価格フロアは実質デッドコードとして走っていた。導入前後で約19万点と変わらなかったのは「安全」ではなく「未測定」を意味していた。PRに「randomではフロアはほぼ発動しない」と自分で書きながら、それを安全の根拠と誤解した。
- 設計上の誤り: このtapeは固定行動列であり、HIRE / BUY_PRODUCT / BUY_SEEDは「その時点で特定の現金がある」前提で記録されている。SELLを絞ると現金の到着タイミングとshed占有が崩れ、後続の購入が連鎖的に失敗する。SELL数量への介入は局所的な変更ではなく、記録された経済全体の同期を壊す変更だった。shed満杯→動物餓死はその一症状にすぎない。
- 自己対戦での追試（結果は撤回に不利）: 撤回版と価格フロア版を同一シード・両席で8戦させたところ、価格フロア版が5勝3敗で僅かに上回り（差1〜3%）、動物も両版とも14頭を維持して餓死は再現しなかった。自己対戦の価格下落（`MELON`→106、`WOOL`→125〜242、`MILK`→26〜101）は実リプレイの暴落（`MELON`→4、`WOOL`→1、`MILK`→1）よりはるかに穏やかで、shedも満杯に張り付かない。つまり自己対戦でも実ラダーの失敗モードは再現できない。
- 判断: ローカル検証（random・自己対戦とも）では価格フロアの実害を確認できないため、根拠は実スコアのみとなる。`AGENTS.md`の「改善候補が下回った場合は安定版を保持する」に従い、`replay_policy.py`と`tests/test_replay_policy.py`をPR #10直前の`298eec3`の内容へ完全に戻した。`_PRICE_REFERENCE` / `_PRICE_FLOOR_TIERS` / `_price_floor_fraction` / `respect_price_floor` / `_SHED_PRESSURE_RELIEF`はすべて削除済み。`_terminal_market`の最終日清算修正も、既知の良好版と完全一致させて読みを濁さないため今回は戻し、再投入は別途単独で測定する。
- 再発防止: `scripts/selfplay_eval.py`を追加した。自己対戦で価格を実際に下げた条件を作り、shed占有ピーク・日次の動物数・最終価格を出力する。市場側の変更は今後これで測定する。ただし上記のとおり自己対戦は実ラダーより暴落が穏やかなので、自己対戦での勝ちは必要条件であって十分条件ではない。次段階の候補は、実際の敗戦リプレイの相手行動列を再生する「記録相手」エージェントを用意し、より現実的な敵対市場で測ること。
- 検証: `python -m pytest -q`は13件成功（価格フロア用テスト4件は撤回に伴い削除、pre-price-floor時点の構成に戻る）。`python scripts/simulate.py --episodes 3`はrandom相手に`185,449 / 189,001 / 184,848`。`python scripts/selfplay_eval.py --episodes 3`ではshed占有ピーク88〜97、動物は最終14頭・11日目以降の最小12頭で、満杯固定と餓死が起きないことを確認した。
