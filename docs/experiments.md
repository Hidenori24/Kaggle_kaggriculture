...
## 2026-08-16: 適応化 第1段 — idle手番の回収（PASS 1,047の内訳を潰す）

- 「1,300超えには適応型になるしかない」の結論を受けて着手。まず**どこが適応の差なのか**を、5件のリプレイの行動列から特定した。
- **農場の大きさは差ではなかった。** `ali dzaki`戦では日次スナップショットで作物55〜60対56〜59、動物14対14とほぼ同一の農場でありながら、最終資金は 67,725 対 94,437。**同じ農場から47%多く現金化されている。**
- **差はactor-stepの使い方だった。** 6,901 actor-stepのうち我々のPASSは1,047。相手は全員それより少ない。

  | 相手 | 相手のPASS | 我々のPASS | `COLLECT_FERTILIZER`(相手/我々) | 対戦結果 |
  | --- | ---: | ---: | ---: | ---: |
  | Manifolds1 | 1,050 | 1,047 | 249 / 254 | −4.5%（最接近） |
  | MJVinay | 813 | 1,047 | 314 / 254 | −6.1% |
  | galaxyhunter | 820 | 1,047 | 312 / 254 | −7.8% |
  | Jacob Alstrup | 625 | 1,047 | 347 / 254 | −21.6% |
  | ali dzaki | 683 | 1,047 | 306 / 254 | −28.3% |

  **PASS量が我々と一致する唯一の相手（Manifolds1）が、唯一の僅差**である。同型相手との差はほぼ`COLLECT_FERTILIZER`1本に集約されていて、tapeの書き起こしがidle部分で薄かった箇所と一致する。
- 売却側も同様に、適応型はSELLを282〜288ステップ・7〜9品目で出す（我々は219・6品目）。
- **却下: shed全品目の投げ売り。** 実測 −21.4% / −23.2% / −10.4%。STRAWBERRY・MELON・MILK・WOOLは価格フロアまで59〜158単位しかなく、注文は自分の単位で価格を押し下げながら約定するため、在庫が小銭に変わったうえ次の売却も壊れる。`ali dzaki`が同じ大量注文で機能するのは、増分がCARROT・TOMATO・EGGという log/sqrt 曲線の品目だからである。
- **却下: 弾力品目（FERTILIZER/CARROT/EGG/TOMATO）に絞った投げ売り。** −22.8% / −21.1% / −10.0%。原因はFERTILIZERで、tapeは`FERTILIZE`を80回行い（水やり日の収量を2倍にする）その1単位をshedから`PICKUP`する。売り切ると80回全部が静かに空振りになる。
- 採用した3層と、それぞれの単独寄与:

  | 実装 | 直接対決（8seed×両席=16戦） |
  | --- | ---: |
  | `_idle_work` 単独（足元の仕事のみ、移動なし） | 14-2、**+0.13%** |
  | `_idle_work` + `_release_fertilizer` | 8-8、**+0.06%** |
  | **上記 + `_idle_route`（往復可能なidle区間だけ肥料へ寄り道）** | **15-1、+1.14%**（中央値 +1.04%） |

  肥料は「集めるだけ」ではshedに滞留し、「全部売る」と自分の増収手段を潰す。**移動して集める**層が入って初めて数字になる。1試合あたりの`COLLECT_FERTILIZER`は片席15回から111回へ増えた。
- `_idle_route`の安全性の担保: tapeは固定リストなのでidle区間の長さが事前に分かる（1,046 idle手番のうち878が長さ4以上、最長115）。**往復が自分のidle区間に収まる場合だけ**離脱し、出発タイルへ必ず戻る。tapeが再び喋るとき actor は tape の期待する位置にいるので、ルーティングは近似ではなく厳密に保存される。
- `_FERTILIZER_RESERVE = 20`。FERTILIZERの上側曲線は linear・target 0.40・T=200 で、余剰1単位あたり価格0.2しか下がらず、フロアまで約500単位ある（MELONやWOOLは二乗）。動物14頭×30日で400単位規模を産めるのに、tapeは176しか売っていなかった。
- **却下: `_idle_route`の寄り道先に「未水やりの作物」を追加。** −3.0% / −1.0%（肥料のみの +1.14% に対して明確な悪化）。原因は寄り道枠の奪い合いで、未水やりタイルは常時50枚以上あるのに対し肥料タイルは少数のため、最近傍選択が actor を軒並み作物側へ吸い寄せる。水やりはtapeが既に922回こなしており限界価値が低い。**寄り道の予算は希少で高価値な用事に使うべき**で、肥料のみに戻した。

## 2026-08-16: 却下 — 第4区画の開墾とニンジン栽培

- 動機は実測に基づいていた。自己対戦で**25タイルが最後までロックされたまま**（`BUY_LAND`は2回で停止）、空きタイルは12日目以降1〜6枚しかなく、farmは飽和している。生産余地は第4区画にしか存在しない。加えて**Jacob Alstrup（−21.6%の相手、動物18頭・20日目作物68）だけが`BUY_LAND`を3回（6日・11日・12日）出しており**、第4区画を開けている。他4人と我々は2回で止まる。
- 作物にニンジンを選んだ理由: 上側曲線が sqrt・T=450 で数百単位を吸収する（MELON/WOOLは二乗で60単位未満で崩れる）。かつ`first_yield_day`が2で最速（STRAWBERRY/MELONは10）で、12日目に開く土地に合う。適応型2人は種を10〜16買い、我々は0。
- 区画の地理も有利に見えた。`LAND_ORDER`はSEを最後に開け、shed隣接タイルはSEの内角にあるため、新しい土地はshedに最も近い。
- **実装3案、すべて失敗:**
  1. **寄り道ルータ（`_idle_route`）に作付け・水やり・収穫を載せる**: tapeのactorはNW側で働くためSEまで往復20手番超が必要で、idle区間が足りない。20枚以上が空のまま季節を終え、自己対戦の報酬は 95k→76k。
  2. **tape外の余剰の手を6人雇う**: 雇用費はその日のn番目に対する`fib(n)`で、tapeが1日最大14人雇うため追加分は15〜20番目＝**1日あたり約16,700**。報酬 12,279（対 95,000）。「雇用はほぼ無料」という当初の見積もりは誤りで、安いのは日の最初の数人だけだった。
  3. **雇用コスト上限150で頭数を自動調整**: 資金破綻は解消（報酬 80,570）が、労働力が足りずニンジンは21日目に0本へ減り空きタイルは17〜21枚に戻る。**直接対決は対 `claude/adaptive-liquidation` で −40.4%。**
- 結論: 第4区画は土地代4,000・種代・雇用費を先払いさせるのに、tapeの労働配分では回収できる規模の作付けを維持できない。**Jacobが第4区画で勝てるのは、農場全体を状態から組み直す計画を持っているからで、固定tapeに区画を足しても労働が付いてこない。** この筋は固定tapeの上では成立しない。
- 実装は`claude/adaptive-replant`ブランチに残してある（未マージ）。
