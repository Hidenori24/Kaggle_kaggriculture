...
## 2026-08-12: exhausting the tape's headroom (negative results)

固定tapeに対する市場側・遊休労働側の介入を、実相手ベンチ（8戦）で一通り測った。**採用に至ったのは谷回避のみ**で、残りは記録として残す。

- 品目別の価格弾力性を環境の`MARKET_PARAMS`から算出した。価格が床に落ちるまでの超過量は品目差が極端で、`WOOL`約59単位（base200, sq）、`STRAWBERRY`約63（base120, linear）、`MILK`約76（base160, linear）、`MELON`約158（base250, sq）に対し、`FERTILIZER`約500、`WHEAT`/`EGG`はlog曲線で事実上無限。実リプレイの`WOOL`→1、`MELON`→4はこれで説明できる。先の一律上限テストが失敗したのは、いくら売っても価格が動かない`WHEAT`まで抑えていたため。
- 品目別の売却ペーシング（脆い4品目のみ上限、現金・shed安全条件付き）: 上限3で`145,995`（−2.05%、動物8）、上限5で`146,439`（−1.75%、動物8）。**品目を正しく選んでも悪化する**。売却を絞ると必ず動物ループが痩せる。
- 結論（市場側）: tapeは現金・shed・飼料が密結合したパイプラインで、売却の量とタイミングを動かす余地はほぼ無い。安全に取れたのは谷回避の+0.30%だけで、これが上限とみてよい。
- 遊休労働の活用: tapeはhand行動枠6,181のうちPASSが1,011（16.4%）、移動が45.9%、実作業が約30%。PASS枠だけを無償の生産行動へ置換する候補を測った（tapeの行動は奪わず、移動もしないのでルートは保たれる）。
  - 給水+世話のみ: **差分ちょうど0**。tapeは立ち位置の給水・世話を既に完全に消化しており、取りこぼしが無いことの裏付けになった。
  - 収穫+給水+肥料回収+世話: 平均`149,371`（+0.21%）、最小`106,898`→`116,208`（**+8.7%**）。ただし11日目以降の動物最小が12→9へ低下する。
  - shed占有ガード（<80 / <60 / <40）を足しても動物は9のまま改善せず、`<40`では−0.97%。したがって動物減の原因はshed競合**ではなく**、未解明のまま。
- 判断: 最悪ケース+8.7%は魅力だが、原因不明の副作用を抱えた変更を出すのは今回の失敗の繰り返しになる。**採用しない**。動物減の機序を特定できたら再検討する。
- 残る上振れ余地はラッパー調整ではなく`_ACTIONS`そのもの（より強いリプレイの取得）か、状態依存ポリシーの再構築である。前者は過去に205→1,755点を生んだ実績のある経路。

## 2026-08-12: top-2 replay analysis (tape transplant is a dead end)

- 入手: リーダーボード1・2位の直接対決2試合。`93221407`（Thomas Tschinkel 148,569 / カワシギ 155,637、seed 1235770897）と`93223214`（カワシギ 94,318 / Thomas 90,229、seed 233458943）。席とseedが入れ替わった同一カード。
- 決定性の判定: 同一エージェントの2試合の行動列を比較したところ、**Thomasは94.6%一致**（初分岐step282、内容は`DIG`と`PLANT WHEAT`の差＝雑草補正）、**カワシギは18.8%一致**（step4で分岐）。カワシギは明確に状態依存。
- **重要な訂正**: 上記2試合はどちらも相手がカワシギである。したがって94.6%が示すのは「seedに対する頑健性」までで、「相手が変わっても同じ」ことは示していない。実際、Thomas自身のseed`233458943`で相手をこちらに変えて再生すると、彼の得点は実試合の`90,229`から`21,087`へ崩れた。盤面は同一なので、崩壊要因は雑草配置ではなく**相手の取引**である。
- 移植の可否（本題）: Thomasのtapeを(1)素で再生、(2)`_ACTIONS`を差し替えて**うちの防護層（雑草補正・SELLクランプ・hand整列）に載せて**再生、の両方で実相手ベンチにかけた。結果は(1)`27,449`、(2)`28,884`（動物は11日目以降最小1頭）で、うちの`149,051`に対し**約−81%**。`_PREEMPT_ENABLED=False`（`_GOLD_HAZARD`はうちのtapeのstep番号に紐付くため）でも同値。**他人のtapeの丸ごと移植は成立しない**と結論する。強さは行動列ではなく、行動列から復元できない適応層にある。
- 一方で理論の裏付けは得られた。SELL単位のstep%4分布は、**Thomasが mod1（町消費直後＝最高値）に908単位・mod0（最安）に249単位**、**うちは mod0 に957単位・mod1 に290単位**でほぼ正反対だった。環境コードから導いた「`_process_market`→`_town_consume`の順序ゆえ mod0 が最安」という機序を、1位のエージェントが実際に体現している。
- ただし retrofit は効かない: 谷回避の効果はうちのtapeでは+0.30%止まりで、ガードを所持金ベースへ緩めても（支出見積の3倍/2倍/1.2倍の余力で判定）`+0.04% / +0.04% / −0.02%`と変化しない。売却タイミング単体ではなく**生産スケジュール全体がmod1に噛み合っていること**が効いているため、既存tapeへの後付けでは取れない。
- その他の構造差: Thomasは`BUY_SEED`108回（うち18回）、`BUY_PRODUCT`71回（うち233回）、hand枠のPASS 9.8%（うち16.4%）、最大hand数12（うち14）。作付けが多く飼料購入が少ない。
- 結論: tape差し替えによる飛躍は少なくともこの2エージェントについては不可能。今後リプレイを入手した場合は、まず「うちの防護層へ載せてベンチ8戦」で3分程度で移植可否を判定できる（`scripts/benchmark.py`＋`_ACTIONS`差し替え）。上振れを狙うなら状態依存ポリシーの再構築が残る唯一の筋だが、ベンチマークが揃った今は初めて安全に反復できる。
