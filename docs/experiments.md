...
## 2026-08-15: challenger candidate — idle-hand work (提出待機)

- 目的: `submission`スロットを2本使い、安定版（`baseline/score-1263`）と**1箇所だけ異なる**版を同時にラダーへ乗せる。ベンチマークがラダーを予測しない問題に対し、実ラダー上でのA/B比較で直接答えを出す。`docs/branch-operation.md`の「最新2件の運用」に沿う。
- 変更点: `_work_idle_hands`を追加。tapeが`PASS`にしているhand枠のみを、その場でできる無償作業（収穫→給水→肥料回収→世話の優先順）へ置換する。tapeの行動は一切奪わず、ユニットを移動もさせないためルートは保たれる。tapeはhand枠6,181のうち1,011（16.4%）を`PASS`にしている。
- ベンチマーク: 平均`149,371`（安定版`149,051`比 **+0.21%**）、最小`116,208`（同`106,898`比 **+8.7%**）、8勝0敗。ラッパー実験時の測定値を正確に再現した。
- ただし単純な上位互換ではない。`ali dzaki`（`106,898`→`116,208`）と`weedfarmer`（`184,415`→`201,270`）は大きく改善する一方、`X1a0Ch3n`は悪化する（`130,189`→`122,186`、`148,795`→`125,220`）。平均はほぼ動かず、**分散の付け替え**に近い。
- 未解明の副作用: 11日目以降の動物最小数が12→9へ低下する。shed占有ガードを付けても回復せず、shed競合が原因ではないことは確認済み。原因不明の変更を単独で安定版に載せるのは避けるべきだが、**別スロットで走らせれば安定版のレーティングは無傷**であり、これはむしろ正しい検証方法である。
- 検証: `python -m pytest -q`は22件成功（遊休活用の5分岐＝収穫・給水・整備済みは放置・肥料回収→世話の優先順・tape行動を絶対に奪わない、を追加）。`ruff`クリーン。
- 提出条件: Kaggleが同時にアクティブ保持できるsubmission本数を先に確認すること。最新1本のみがアクティブになる方式の場合、本版をpushすると安定版が非アクティブ化し、収集中のレーティングデータが途切れる。確認が取れるまでpushしない。

## 2026-08-15: challenger v2 — idle work + feed reserve (機序特定して副作用を解消)

- 前版（遊休活用のみ）の未解明副作用「11日目以降の動物最小が12→9」を、両版を同一試合でトレースして特定した。`ali dzaki`戦で7日目に分岐し、6日目のshed内WHEATがベースライン`6`に対し挑戦版`1`、牛1頭が`consecutive_unfed=1`で失われていた。
- **機序**: 遊休収穫でWHEATが増える（4日目に`5`→`16`）→ tapeの`SELL WHEAT`は`_safe_market`でshed在庫までしか通らないため、**在庫が増えた分だけ余計に売れる**（6日目に`6`→`1`）→ 飼料が尽きて餓死。tapeの売却量は自分の生産量前提で調整されており、飼料の取り置きをしない。つまり遊休収穫が**tapeの売り注文に燃料を与えて飼料を売り払わせていた**。
- 外れた仮説も記録する。当初は日替わり処理`_drop_inventories_to_shed`（856行、運搬者の手持ちを全部shedへ流し込み超過分は破棄）による圧迫を疑い、shed＋運搬者在庫の合計に予算（90/80/70/60）を課したが、動物は9のまま回復せず平均は単調に悪化した。機序はshed容量ではなく**飼料の売却**だった。
- 対策と切り分け（実相手ベンチ8戦）:
  | 構成 | 平均 | 最悪 | 動物最小 |
  | --- | ---: | ---: | ---: |
  | 安定版 | 149,051 | 106,898 | 12 |
  | 遊休活用のみ | 149,371 | 116,208 | 9 |
  | 飼料取り置きのみ（2日分） | **144,524（−3.0%）** | 106,073 | 11 |
  | WHEATだけ収穫しない | 148,908 | 106,966 | 12 |
  | **遊休活用＋取り置き2日分** | **152,014（+1.99%）** | 114,575 | 11 |
  | 同 3日分 | 143,772 | 110,759 | 11 |
  | 取り置きのみ3日分 | 127,881 | 105,469 | 10 |
- **どちらも単独では効かない**。取り置き単独は売る物を減らすだけで−3.0%、遊休活用単独は飼料を売らせるだけで+0.2%。両者は相互作用しており、遊休収穫が余剰WHEATを生み、取り置きがそれを売り払われるのを防いで初めて+1.99%になる。
- 内訳は一様な改善ではない。**改善5戦・悪化3戦**で、`Mehmet seat0 +10,778` / `ali dzaki seat0 +7,677` / `weedfarmer 両席 +8,715` が伸びる一方、`Mehmet seat1 −6,984` / `X1a0Ch3n seat1 −4,696` / `ali dzaki seat1 −4,506` が下がる。**seat0では4戦とも改善、seat1では3戦悪化**という席依存の偏りがあり、原因は未調査。
- 実装: `_FEED_RESERVE_DAYS = 2`と`_reserve_feed_wheat`を追加し、`_safe_market`の末尾で適用（測定時と同じ位置）。最終日の`_terminal_market`は`_safe_market`の後に走るため、取り置き分も含めて全量清算される。
- 検証: `python -m pytest -q`は25件成功（取り置きの境界・頭数比例・動物ゼロ時の非適用を追加）。`ruff`クリーン。統合実装がベンチ`152,014`を正確に再現。
- 位置づけ: 2本目のsubmissionスロット用の挑戦版。安定版`baseline/score-1263`との差分は「遊休hand活用」と「飼料取り置き」の2点。実ラダーで安定版と直接比較する。

## 2026-08-15: 挑戦版を却下 — ベンチマークが構造的に誤る方向を特定

- 問題提起: 実ラダーのレーティングが1,300台で上下し始めた＝その帯で勝ったり負けたりしている。ならば「すでに勝っている相手への勝ち幅」を改善しても順位は動かない。この対策は**どこに効くのか**を確認した。
- ベンチマークの実態: 挑戦版・安定版とも実相手ベンチは**8戦8勝**で、勝ち幅は`149k`対`65〜114k`。つまり測っているのは「勝つ試合の勝ち幅」であって、接戦の勝敗ではない。実際の敗戦（`93332940`）は`38,455`対`40,945`の**−6%**であり、まったく別の領域である。
- 決定的な検証: 挑戦版と安定版を12seed×両席の**24戦で直接対決**させた。結果は**挑戦版 0勝24敗、平均マージン −10.72%**（中央値 −9.50%、全試合で敗北）。ベンチの`+1.99%`は完全に見せかけだった。
- 機序: ベンチの相手は記録tapeの再生であり弱体化しているうえ、**市場を奪い合わない**。したがって自分の生産・売却を増やす変更は、空いた市場へそのまま売れて勝ち幅が伸び、必ず良く見える。一方、互角の相手と同一市場を取り合うと、増やした収穫物は自分の売値を潰し、飼料取り置きは売れるはずのWHEATを抱え込む。差し引きで大幅な損。
- **結論: 挑戦版は提出しない。** ベンチマーク単独をゲートにしてはならない。
- 対策: `scripts/head_to_head.py`を追加した。作業ツリーのポリシーを任意のgit revisionの自分自身と、両席・多seedで対戦させる。接戦かつ市場を奪い合う領域を再現するので、ラダーの1,300帯に近い。**自分の前任に勝てない変更は提出しない**を新しいゲートとする。
- 留保: 自己対戦もラダーそのものではない（相手は常に自分の亜種であり、実際の対戦相手ではない）。ただしベンチマークより厳密に近い。今後は「ベンチマーク（回帰検出）＋直接対決（採否判定）」の二段構えとする。
- 未解決: この試験は価格崩壊を再現しない（崩壊品目 平均0/9）。1,300帯の実敗戦は全面崩壊局面で起きているため、そこを測る手段は依然として無い。
