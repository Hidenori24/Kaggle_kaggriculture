...
## 2026-08-15: submission-49 — price floor removed (安定版として保存)

- 提出: Actions Run `31888785142` / job `95021813953` 成功。`submission`ブランチのコミット`6cbaeb0910b005f3048180930a073cba6c3cd9ab`。
- パッケージSHA-256: `76d5fc56b77807c9676834c529ab26b82f6ace0f2efb9642b4d1b4ed578f9ac5`（25,960 bytes）。Kaggle APIは`Successfully submitted to Kaggriculture`、当日残り4回。
- 中身: 価格フロア一式を削除し、実測で勝った`_shift_sells_off_trough`のみ採用。`submission`固有だった`_shadow_market_overlay`はエージェント経路から外した（旧ベースで+112＝誤差、新ベース上では未測定のため）。`economic_shadow.py`はパッケージとテストに残置。
- 提出時ベンチマーク: 平均`149,051`、最小`106,898`、8勝0敗、全試合で動物は最終14頭・11日目以降の最小12頭。差し替え前の提出版（価格フロア＋shadow）が`145,358`だったので **+2.5%**。
- ラダー実測（暫定）: 7連勝でレーティング約`1,263`。**まだ連勝中＝実力値より下にいる**ため最終的な上げ止まり値は未確定。ただし置き換え前の価格フロア版が約`1,000`で**停滞していた**（＝あれが実力値）ため、**フロア版の実力を上回ったことは確定**した。
- したがって「価格フロアが約900点を奪っていた」という診断と、実相手ベンチマークの方向性は実測で裏付けられたと判断する。判定ラインは「30戦以上で上げ止まった値が1,200超なら成功」としており、現時点でこれを満たす見込み。
- 保存: 本コミットを`baseline/score-1263`ブランチとタグ`submission-49`で凍結する。以後の実験はこのベースラインを直接変更しない。上げ止まり値が確定した時点で、必要なら最終スコア名のタグを追加する。
- 注意: `1,900`（過去のピーク`2,500`）を基準にはしない。コード変更なしで`2,500`→`1,900`へ下落した実績があり、相手プール自体が強くなっているため、同一エージェントでも当時の水準には届かない可能性がある。比較対象は同時期の価格フロア版`1,000`のみが妥当。
