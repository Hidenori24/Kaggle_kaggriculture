...
## 2026-08-15: submission-49 ladder result & a benchmark limitation

- ラダー実測: 提出後7連勝でレーティング約`1,263`まで上昇し、その後`93332940`で初黒星（自分`38,455` vs Francesco Montone `40,945`、−6%の僅差）。置き換え前の価格フロア版が約`1,000`で**停滞していた**のに対し、本版は1,263到達後に負け始めた＝ようやく実力値付近に達したところ。**フロア版の実力を上回ったことは確定**。
- 敗戦`93332940`の構造: 市場が全面崩壊した試合。`MELON`250→1、`WOOL`200→1（16日目時点で既に1）、`MILK`160→5、`STRAWBERRY`120→59（24日目に1）、`FERTILIZER`100→3。一方で非弾力品目は逆に上昇し、`EGG`50→154、`WHEAT`25→54、`TOMATO`60→68。
- 収支分解: 総収入は**うちが上回る**（`89,530` vs `81,094`）。負けたのは支出差（`54,156` vs `36,879`）。内訳では相手が`STRAWBERRY`27,213（うち15,804）と`FERTILIZER`16,571（うち8,842）で約2倍稼いでいる。この試合で唯一崩れなかった品目を多く生産していたということで、**市場タイミングではなく作付け構成の差**。tapeを書き換えない限り触れない領域である。
- WHEAT取引の再評価: 899単位を平均`39.4`で購入し745単位を平均`42.9`で売却しており、飼料154単位の実効単価は約`22`。`WHEAT`価格が25→54へ上昇する相場では、この売買は飼料代を補助しており**無駄ではない**。以前「ウォッシュ取引」と見なした判断は誤りで、削除実験が−44%だったのはこのためと整合する。
- **ベンチマークの限界（重要）**: この敗戦相手をベンチへ追加して測定したところ、彼の再生tapeは`3,958`しか取れず（実試合では`40,945`）、市場崩壊も再現されなかった（全品目が高値のまま）。うちの得点は`196,731 / 198,725`で退屈な圧勝になる。すなわち**状態依存型の相手ほど再生で弱体化し、その相手が実際に市場へ与えた圧力を再現できない**。したがって本ベンチマークは市場崩壊シナリオを系統的に過小評価する。追加は取り消し、基準の連続性を優先して4体（8戦）のまま維持する。
- 含意: 価格に依存する変更の評価には依然として上限がある。実ラダーでしか出ない全面崩壊局面を測りたい場合は、記録相手の再生ではなく、崩壊した価格系列を人工的に与える合成シナリオが必要になる。
