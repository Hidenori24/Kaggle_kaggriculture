...
## 2026-08-16: 1,300の天井 — 原因の確定と、なぜ2%では届かないか

- 谷回避の除去（PR #16、直接対決32勝0敗・+2.32%）を提出したが、**レーティングは1,300を超えなかった**。実際の敗戦から機序を特定して直した修正でも天井は動かない。
- **理由は算数である。** 敗戦5件のマージンは `−4.5% / −6.1% / −7.8% / −21.6% / −28.3%`。**+2%の改善では1件も勝ちに転じない**。+5%で1件、+8%で3件が反転する計算になる。今セッションのラッパー改良はすべて2%前後であり、必要な水準に対して桁が1つ足りていない。
- 8〜26%の差が実在する場所は1つだけで、12日目以降の日次資金増加（うちは相手の73.7〜93.0%）である。これは生産効率そのものであり、tapeの構造に属する。
- **対戦相手の分類が確定した。** 5件の敗戦相手の行動列を相互比較したところ、明確に2群に分かれた。
  | 群 | 相手 | 相互一致率 | 対戦成績 |
  | --- | --- | ---: | ---: |
  | うちと同じ固定tape | MJVinay / Manifolds1 / galaxyhunter | 62〜83% | −4.5〜−7.8%（ほぼ互角） |
  | 適応型 | Jacob Alstrup / ali dzaki | 相互30.1%（農場計画） | **−21.6 / −28.3%** |
  - 前者はうちのtapeと同型で、序盤投資も作付けタイルも完全一致する。後者は序盤投資が異なり（HIRE 81対71、WHEAT種35対21、CARROT種8対0）、step 180以降は移動先も作業内容も互いに食い違う。**適応型である。**
  - `Thomas Tschinkel`・`カワシギ`（1・2位）も同じく適応型であることを確認済み。
- **適応型のtapeは移植できない。** Thomasのtapeをうちの防護層に載せた結果は−81%（動物は最終1頭）。強さは行動列ではなく、行動列から復元できない適応ロジックにある。今回の`Jacob`/`ali dzaki`も相互一致30.1%であり、固定tapeとして書き起こす対象が存在しない。
- **結論: 固定tape方式の天井は約1,300で確定。** 内訳は「同型の相手とはほぼ互角、適応型の相手に−21〜−28%で負ける」であり、勝率50%がこの帯に落ち着く。ラッパーで触れる範囲（市場注文の並び順・数量・タイミング）はすべて2%規模で、この構図を変えられない。1,300を超えるには**適応型になる以外に道はない**。
- 今セッションの正味の成果: 実スコアを約1,000から1,300帯へ回復させた（有害だった価格フロアの除去が主因）。加えて測定基盤3層（`benchmark.py`＝回帰検出、`head_to_head.py`＝採否判定、`stress_market.py`＝崩壊相場）と、それぞれの盲点を実測で確定させた。`scripts/slim_replay.py`により、30MBのリプレイを約230KBへ縮約して継続的にデータを取り込めるようにした。
- 未着手の課題として、序盤の投資テンポ差が残る。敗戦2件では4日目にうちが相手の**1067%**の資金を保持しているのに（相手は使い切っている）、12日目には58〜61%へ逆転され以降固定される。適応型を実装する際は、この「序盤に重く投資し、中盤以降の生産力に変える」挙動が最初の再現対象になる。
