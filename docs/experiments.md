...
## 2026-08-07: adaptive-score-safe-candidate

- 目的: 1882.7相当の基準版を壊さず、勝敗に直結する終盤の未売却在庫だけを回収する。
- 実装: 通常720ターンのうち696ターン目までは`replay_policy`を完全に使用し、最終24ターンだけ`_terminal_market`で納屋の売却可能在庫を市場注文へ追加する。売却量は観測された在庫を上限とし、市場注文上限10件も再適用する。
- 意図的に未実装: 通常局面の雇用・土地・作物・動物・市場価格の変更。基準版の勝ち筋を保ったまま、まず終盤回収だけを比較する。
- 公式仕様上、勝敗は720ターン終了時の銀行残高で決まり、未売却在庫は加算されない。またLeaderboardのレーティングは得点差ではなく勝敗で変化するため、最初の候補はこの安全弁に限定する。
- 比較: 同一5シードのrandom対戦では基準版`191,914.2`に対し候補`193,792.8`（`+1,878.6`）だったが、候補の改善は2/5シードに留まり、3/5シードでは低下した。基準版との直接対戦でも候補の勝敗が席に依存したため、現時点ではmain/submissionへ昇格しない。

## 2026-08-07: pass-repair-experiment

- 目的: 固定行動列の`PASS`を、現在位置で安全に実行できる局所作業だけへ置換し、中盤以降の手数の取りこぼしを減らす。
- 実装: `pass_repair_policy.py`を追加。予定行動が`PASS`の場合に限り、成熟作物の`HARVEST`、未給水作物の`WATER`、生産物のある動物施設の`COLLECT_FERTILIZER`だけを候補にする。移動、植付け、市場、雇用、土地購入、動物購入は変更しない。
- 単体テスト: `python -m pytest -q` は17件成功。
- 同一10シードの比較:
  - 候補: 187,423 / 174,133 / 195,337 / 193,615 / 190,668 / 195,381 / 204,585 / 198,765 / 200,121 / 197,722、平均193,775。
  - 基準: 179,932 / 186,673 / 183,408 / 204,562 / 199,286 / 192,665 / 198,419 / 188,515 / 195,572 / 194,631、平均192,366。
  - 候補は6/10シードで上回ったが、4/10シードでは低下し、安定性の改善は確認できなかった。
- 判断: 固定ルートの同期を崩す副作用があるため、`main`および`submission`へ昇格せず、実験ブランチに留める。Kaggleへは提出しない。
- 追加20エピソード評価: 候補平均は約`195,400`、基準版平均は約`195,836`。候補では相手報酬が発生する試合もあり、サンプル数を増やしても安定優位は確認できなかった。候補は不採用とし、次回提出は既存の基準版を使用する。
