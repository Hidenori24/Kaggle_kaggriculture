...
def _preempt_profile(distance):
    for limit, fraction, batch in _PREEMPT_LEVELS:
        if distance <= limit:
            return fraction, batch
    return 0.0, 0
...
def _preempt_shift(obs, action, step):
    ...
    if state.get("due") or step - int(state.get("last_preempt", -10**9)) < _PREEMPT_COOLDOWN:
        return action
    clone_distance = _clone_distance(obs)
    if clone_distance > _PREEMPT_MAX_CLONE_DISTANCE:
        return action
    preempt_fraction, preempt_batch = _preempt_profile(clone_distance)
    if preempt_fraction <= 0.0:
        return action
    ...
    for item in _PREMIUM:
        ...
        target = min(
            ...
            preempt_batch,
            max(1, int(round(float(future_base.get(item, 0)) * preempt_fraction))),
        )
        if target <= 0:
            continue
        existing_index = next(
