...
def load_policy(name):
    ...
    if name == "legacy":
        ...
        return agent
    if name == "crop-mix":
        from kaggriculture_agent.crop_mix_policy import agent
        return agent
    if name == "carrot-mix":
        from kaggriculture_agent.carrot_mix_policy import agent
        return agent
    if name == "carrot-lane":
    ...
    if name == "stateful":
        ...
        return agent
    if name == "macro":
        from kaggriculture_agent.macro_transport import agent
        return agent
    if name == "local":
        from kaggriculture_agent.local_jobs_policy import agent
        return agent
    if name == "late":
        from kaggriculture_agent.late_harvest_policy import agent
        return agent
    if name == "resync":
        from kaggriculture_agent.resync_policy import agent
        return agent
    if name == "pressure":
        from kaggriculture_agent.pressure_order_policy import agent
        return agent
    if name == "wheat-budget":
        from kaggriculture_agent.wheat_budget_policy import agent
        return agent
...
def main():
    ...
    parser.add_argument(
        "--policy", choices=("replay", "legacy", "crop-mix", "carrot-mix", "carrot-lane", "full", "baseline", "stateful", "macro", "local", "late", "resync", "pressure", "wheat-budget", "redundant-feed", "daily-economic", "input-budget"), default="replay",
